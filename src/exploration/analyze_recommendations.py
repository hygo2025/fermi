...
def create_visualizations(
    ...
    if results_df is not None and 'avg_similarity' in results_df.columns:
        ...
        if len(similarities) > 0:
            ...
            axes[0].set_xlabel('Similaridade Ponderada (%)', fontsize=11)
            ...
            axes[0].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[0].transAxes)
            axes[0].set_title('Distribuicao de Similaridade', fontsize=13, fontweight='bold')
    else:
        axes[0].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[0].transAxes)
        axes[0].set_title('Distribuicao de Similaridade', fontsize=13, fontweight='bold')
    ...
    if 'price' in sess_df.columns and 'price' in rec_df.columns:
        ...
        if len(sess_prices) > 0 and len(rec_prices) > 0:
            axes[1].hist(sess_prices, bins=10, alpha=0.6, label='Sessao', color='blue', edgecolor='black')
            axes[1].hist(rec_prices, bins=10, alpha=0.6, label='Recomendacoes', color='red', edgecolor='black')
            
            sess_mean = sess_prices.mean()
            rec_mean = rec_prices.mean()
            axes[1].axvline(sess_mean, color='blue', linestyle='--', linewidth=2, label=f'Media sessao: R$ {sess_mean:,.0f}')
            axes[1].axvline(rec_mean, color='red', linestyle='--', linewidth=2, label=f'Media recs: R$ {rec_mean:,.0f}')
    ...
    if area_col in sess_df.columns and area_col in rec_df.columns:
        ...
        if len(sess_areas) > 0 and len(rec_areas) > 0:
            ...
            axes[2].text(1, sess_median, f'{sess_median:.0f}m2', ha='left', va='center', fontweight='bold', fontsize=9)
            axes[2].text(2, rec_median, f'{rec_median:.0f}m2', ha='left', va='center', fontweight='bold', fontsize=9)
        else:
            axes[2].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[2].transAxes)
            axes[2].set_title('Distribuicao de Areas', fontsize=13, fontweight='bold')
    else:
        axes[2].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[2].transAxes)
        axes[2].set_title('Distribuicao de Areas', fontsize=13, fontweight='bold')
    
    # 4. Gráfico de barras - Quartos
    if 'bedrooms' in sess_df.columns and 'bedrooms' in rec_df.columns:
        sess_bedrooms = sess_df['bedrooms'].value_counts().sort_index()
        rec_bedrooms = rec_df['bedrooms'].value_counts().sort_index()
        
        if len(sess_bedrooms) > 0 and len(rec_bedrooms) > 0:
            ...
            axes[3].bar(x - width/2, sess_counts, width, label='Sessao', color='blue', alpha=0.7)
            axes[3].bar(x + width/2, rec_counts, width, label='Recomendacoes', color='red', alpha=0.7)
            
            axes[3].set_xlabel('Numero de Quartos', fontsize=11)
            axes[3].set_ylabel('Frequencia', fontsize=11)
            axes[3].set_title('Distribuicao de Quartos', fontsize=13, fontweight='bold')
            axes[3].set_xticks(x)
            axes[3].set_xticklabels([f'{int(b)}' for b in all_bedrooms])
            axes[3].legend(fontsize=9)
            axes[3].grid(True, alpha=0.3, axis='y')
        else:
            axes[3].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[3].transAxes)
            axes[3].set_title('Distribuicao de Quartos', fontsize=13, fontweight='bold')
    else:
        axes[3].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[3].transAxes)
        axes[3].set_title('Distribuicao de Quartos', fontsize=13, fontweight='bold')
    
    # 5. Gráfico de barras - Banheiros
    if 'bathrooms' in sess_df.columns and 'bathrooms' in rec_df.columns:
        sess_bath = sess_df['bathrooms'].value_counts().sort_index()
        rec_bath = rec_df['bathrooms'].value_counts().sort_index()
        
        if len(sess_bath) > 0 and len(rec_bath) > 0:
            ...
            axes[4].bar(x - width/2, sess_counts, width, label='Sessao', color='blue', alpha=0.7)
            axes[4].bar(x + width/2, rec_counts, width, label='Recomendacoes', color='red', alpha=0.7)
            
            axes[4].set_xlabel('Numero de Banheiros', fontsize=11)
            axes[4].set_ylabel('Frequencia', fontsize=11)
            axes[4].set_title('Distribuicao de Banheiros', fontsize=13, fontweight='bold')
            axes[4].set_xticks(x)
            axes[4].set_xticklabels([f'{int(b)}' for b in all_bath])
            axes[4].legend(fontsize=9)
            axes[4].grid(True, alpha=0.3, axis='y')
        else:
            axes[4].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[4].transAxes)
            axes[4].set_title('Distribuicao de Banheiros', fontsize=13, fontweight='bold')
    else:
        axes[4].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[4].transAxes)
        axes[4].set_title('Distribuicao de Banheiros', fontsize=13, fontweight='bold')
    
    # 6. Gráfico de barras - Vagas
    if 'parking_spaces' in sess_df.columns and 'parking_spaces' in rec_df.columns:
        sess_park = sess_df['parking_spaces'].value_counts().sort_index()
        rec_park = rec_df['parking_spaces'].value_counts().sort_index()
        
        if len(sess_park) > 0 and len(rec_park) > 0:
            ...
            axes[5].bar(x - width/2, sess_counts, width, label='Sessao', color='blue', alpha=0.7)
            axes[5].bar(x + width/2, rec_counts, width, label='Recomendacoes', color='red', alpha=0.7)
            
            axes[5].set_xlabel('Numero de Vagas', fontsize=11)
            axes[5].set_ylabel('Frequencia', fontsize=11)
            axes[5].set_title('Distribuicao de Vagas', fontsize=13, fontweight='bold')
            axes[5].set_xticks(x)
            axes[5].set_xticklabels([f'{int(p)}' for p in all_park])
            axes[5].legend(fontsize=9)
            axes[5].grid(True, alpha=0.3, axis='y')
        else:
            axes[5].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[5].transAxes)
            axes[5].set_title('Distribuicao de Vagas', fontsize=13, fontweight='bold')
    else:
        axes[5].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[5].transAxes)
        axes[5].set_title('Distribuicao de Vagas', fontsize=13, fontweight='bold')
    
    # 7. Gráfico de barras - Tipo de Unidade
    if 'unit_type' in sess_df.columns and 'unit_type' in rec_df.columns:
        sess_types = sess_df['unit_type'].value_counts()
        rec_types = rec_df['unit_type'].value_counts()
        
        if len(sess_types) > 0 and len(rec_types) > 0:
            ...
            axes[6].bar(x - width/2, sess_counts, width, label='Sessao', color='blue', alpha=0.7)
            axes[6].bar(x + width/2, rec_counts, width, label='Recomendacoes', color='red', alpha=0.7)
            ...
            axes[6].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[6].transAxes)
            axes[6].set_title('Distribuicao de Tipos', fontsize=13, fontweight='bold')
    else:
        axes[6].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[6].transAxes)
        axes[6].set_title('Distribuicao de Tipos', fontsize=13, fontweight='bold')
    
    # 8. Gráfico de barras - Status de Similaridade
    if results_df is not None and 'similarity_status' in results_df.columns:
        ...
        if len(status_counts) > 0:
            ...
            axes[7].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[7].transAxes)
            axes[7].set_title('Distribuicao de Status', fontsize=13, fontweight='bold')
    else:
        axes[7].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[7].transAxes)
        axes[7].set_title('Distribuicao de Status', fontsize=13, fontweight='bold')
    ...
    if results_df is not None:
        ...
        if len(metrics_scores) > 0:
            ...
            axes[8].text(0.5, 0.5, 'Sem dados', ha='center', va='center', transform=axes[8].transAxes)
            axes[8].set_title('Score por Metrica', fontsize=13, fontweight='bold')
    else:
        axes[8].text(0.5, 0.5, 'Dados nao disponiveis', ha='center', va='center', transform=axes[8].transAxes)
        axes[8].set_title('Score por Metrica', fontsize=13, fontweight='bold')
    plt.tight_layout()
