...
class RecBoleConverter:
    ...
    def convert_simple(self):
        """Convert simple train/test split (no slices)"""
        self.log("Converting simple train/test split...")
        
        train_path = self.input_path / "train"
        test_path = self.input_path / "test"
        
        if not train_path.exists() or not test_path.exists():
            raise FileNotFoundError(f"Train or test directory not found in {self.input_path}")
    ...
    def run(self):
        """Converte todos os slices ou simple train/test"""
        self.log("="*80)
        self.log("RecBole Data Converter")
        self.log("="*80)
        
        # Check if simple format (train/test dirs) or sliced format (slice_* dirs)
        has_simple = (self.input_path / "train").exists() and (self.input_path / "test").exists()
        slices = sorted([
        ...
        if has_simple and not slices:
            self.log("Detected simple train/test format")
            print()
            datasets = self.convert_simple()
        elif slices:
            ...
            for slice_id in slices:
                ...
                print()
            datasets = [f"realestate_slice{s}" for s in slices]
        else:
            raise FileNotFoundError(f"No valid data found in {self.input_path}")
        ...
        for dataset in datasets:
            output_dir = self.output_path / dataset
            print(f"  {output_dir}/")
