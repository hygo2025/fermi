...
# Lazy initialization para evitar erro quando variáveis de ambiente não estão configuradas
_CEP_CACHE_PATH = None

def get_cep_cache_path():
    global _CEP_CACHE_PATH
    if _CEP_CACHE_PATH is None:
        geo_path = geo_data_path()
        if geo_path:
            _CEP_CACHE_PATH = os.path.join(geo_path, "cep_geocache.csv")
            ensure_directory_exists(_CEP_CACHE_PATH)
        else:
            _CEP_CACHE_PATH = "cep_geocache.csv"  # fallback
    return _CEP_CACHE_PATH
...
def load_cep_cache() -> pd.DataFrame:
    cache_path = get_cep_cache_path()
    print(f"\nCarregando cache de CEPs de: {cache_path}")
